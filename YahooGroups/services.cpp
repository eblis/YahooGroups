	...
	*group = 1;
	...
	if (bUnicodeMiranda)
	{
		...
		*wide = 1;
		...
		DBWriteContactSettingString(NULL, CLIST_GROUPS, tmp, group);
	}
	
	availableGroups.Add(_strdup(group + 1));
}

void AddContactToGroup(HANDLE hContact, char *group)
{
	if (bUnicodeMiranda)
	{
		const int MAX_SIZE = 1024;
...
int YahooGroupsMoveService(WPARAM wParam, LPARAM lParam)
{
...
int YahooGroupsResetService(WPARAM wParam, LPARAM lParam)
{
